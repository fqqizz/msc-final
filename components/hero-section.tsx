...
const HeroContent = memo(function HeroContent({ isVisible }: { isVisible: boolean }) {
  ...
  const fadeUpVariants = {
    hidden: { opacity: 0, y: prefersReducedMotion ? 0 : isMobile ? 12 : 20 },
    visible: (delay: number) => ({
      opacity: 1,
      y: 0,
      transition: {
        duration: prefersReducedMotion ? 0.1 : isMobile ? 0.45 : 0.6,
        delay: prefersReducedMotion ? 0 : isMobile ? delay * 0.4 : delay,
        ease: [0.16, 1, 0.3, 1],
      },
    }),
  }
  ...
  return (
    <div 
      className="relative z-20 h-full flex flex-col items-center justify-center px-4 sm:px-6 transform-gpu max-w-4xl mx-auto"
      style={GPU_ACCELERATED}
    >
      {/* Main heading — Desktop Restored, Mobile Controlled */}
      <motion.div
        initial="hidden"
      ...
      >
        <h1 className="font-[family-name:var(--font-anton)] text-5xl xs:text-[3.25rem] sm:text-6xl md:text-7xl lg:text-8xl xl:text-9xl tracking-tight text-white leading-[0.9] select-none">
          MAQBOOL
        </h1>
        <h2 className="font-[family-name:var(--font-anton)] text-4xl xs:text-[2.65rem] sm:text-5xl md:text-6xl lg:text-7xl xl:text-8xl tracking-tight text-[#2BA84A] leading-[0.9] mt-1 sm:mt-2 select-none">
          SPORTS COMPLEX
        </h2>
      </motion.div>
      ...
      {/* CTAs (Compact, centered, elegant non-bold UI weight) */}
      <motion.div
        initial="hidden"
        animate={isVisible ? 'visible' : 'hidden'}
        variants={fadeUpVariants}
        custom={0.3}
        className="mt-8 sm:mt-10 flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4 w-full max-w-[280px] xs:max-w-[300px] sm:max-w-none px-4 sm:px-0 transform-gpu"
      >
        <Link
          href="/book-now"
          className="clay-button-green w-full sm:w-auto px-6 sm:px-8 py-3.5 sm:py-4 text-white font-semibold text-xs sm:text-sm rounded-xl text-center shadow-lg transition-transform duration-200 active:scale-[0.98] tracking-wider"
        >
          Book Your Slot
        </Link>
        <Link
          href="/facilities"
          className="w-full sm:w-auto px-6 sm:px-8 py-3.5 sm:py-4 bg-white/10 md:bg-white/5 md:backdrop-blur-sm border border-white/15 hover:border-white/30 text-white font-medium text-xs sm:text-sm rounded-xl hover:bg-white/15 transition-all duration-200 flex items-center justify-center gap-2 shadow-lg shadow-black/40 active:scale-[0.98] tracking-wider"
        >
